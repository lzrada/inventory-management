...
const Page = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get("/api/kelas/ix/c");
      setItems(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put("/api/kelas/ix/c", payload);
        Swal.fire("Success", "Data berhasil diperbarui!", "success");
      } else {
        await axios.post("/api/kelas/ix/c", payload);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete("/api/kelas/ix/c", { data: { id } });
          fetchItems();
          Swal.fire({
            title: "Deleted!",
  ...
  return (
    <div className="flex relative">
      ...
      <div className="flex flex-col p-4 flex-grow">
          ...
          <div className="relative flex-col text-left mt-3">
            ...
            <div className="flex gap-3">
              <div className="w-11 h-11 border border-gray-400 rounded-xl mt-4">
                <Link href="/masterData/kelas/ix/a" className="w-full h-full flex object-fill items-center justify-center">
                  A
                </Link>
              </div>
              <div className="w-11 h-11 border border-gray-400 rounded-xl mt-4">
                <Link href="/masterData/kelas/ix/b" className="w-full h-full flex object-fill items-center justify-center">
                  B
                </Link>
              </div>
              <div className="w-11 h-11 border border-gray-400 rounded-xl mt-4">
                <Link href="/masterData/kelas/ix/c" className="w-full h-full flex object-fill items-center justify-center">
                  C
                </Link>
              </div>
            </div>
        ...
        {/* Tabel Ruangan */}
        <h2 className="flex justify-center text-3xl font-bold my-3">Data Master Kelas IX C</h2>
