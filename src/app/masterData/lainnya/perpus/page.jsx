...
const Page = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await axios.get("/api/lainnya/perpus");
      setItems(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put("/api/lainnya/perpus", payload);
        Swal.fire("Success", "Data berhasil diperbarui!", "success");
      } else {
        await axios.post("/api/lainnya/perpus", payload);
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete("/api/lainnya/perpus", { data: { id } });
          fetchItems();
          Swal.fire({
            title: "Deleted!",
  ...
  return (
    <div className="flex relative">
      ...
      <div className="flex flex-col p-4 flex-grow">
        ...
        {/* Tabel Ruangan */}
        <h2 className="flex justify-center text-3xl font-bold my-3">Data Master Ruang Perpus</h2>
